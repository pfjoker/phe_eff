...
void StMyAnalysisMaker::initHisto()
{
    ...
    Int_t htpce_bin[5] = {3, 10, 100, 300, 100};
    Double_t htpce_xmin[5] = { -1.5, -0.5, 0., -1.5, -TMath::Pi()};
    Double_t htpce_xmax[5] = {1.5, 9.5, 10., 1.5, TMath::Pi()};
    htpce = new THnSparseF("htpce", "sign:cent:pt:eta:phi", 5, htpce_bin, htpce_xmin, htpce_xmax);
    htpce->Sumw2();

    Int_t htofmatch_e_bin[5] = {3, 10, 100, 300, 100};
    Double_t htofmatch_e_xmin[5] = { -1.5, -0.5, 0., -1.5, -TMath::Pi()};
    Double_t htofmatch_e_xmax[5] = {1.5, 9.5, 10., 1.5, TMath::Pi()};
    htofmatch_e = new THnSparseF("htofmatch_e", "sign:cent:pt:eta:phi", 5, htofmatch_e_bin, htofmatch_e_xmin, htofmatch_e_xmax);
    htofmatch_e->Sumw2();
...
Int_t StMyAnalysisMaker::Make()
{
    ...
    for (Int_t i = 0; i < Ntracks; i++) {
        ...
        Current_nCanE++;
        bool smd_cut = (nEta > 0 && fabs(trackZDist) < 2) || (nPhi > 0 && fabs(trackPhiDist) < 0.01);
...
Int_t StMyAnalysisMaker::makephe()
{
    ...
    Double_t e_ptetaphi[5] = {0., 0., 0., 0., 0.};
    Double_t p = 0.0;
    Double_t nSigE = 0.;
    Double_t nse_p = 0.;
    Bool_t nse_cut = 0;
    ...
    for (int i = 0; i < Current_nCanE; i++) {
        ...
        for (int j = 0; j < Current_nCanGE; j++) {
            ...
            if (pairmass < 0.01 && (charge1 * charge2 == -1)) {
                pairdca = diff.mag();
            ...
            if (pairmass < 0.01 && pairdca < 1) {
                ...
                e_ptetaphi[0] = sign;
                e_ptetaphi[1] = mCent1;
                e_ptetaphi[2] = Current_CanEPt[i];
                e_ptetaphi[3] = Current_CanEEta[i];
                e_ptetaphi[4] = Current_CanEPhi[i];

                htpce->Fill(e_ptetaphi, mCentWeight);

                if (Current_CanEBeta[i] > (0 + 1e-4)) {
                    htofmatch_e->Fill(e_ptetaphi, mCentWeight);
